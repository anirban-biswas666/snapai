...
const GenerateImages = () => {
 ...
 const onSubmitHandler = async (e) =>{
  ...
  return (
   <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <p className='mt-6 text-sm font-medium'>Descrribe your image</p>
          ...
          <textarea onChange={(e)=>setInput(e.target.value)} value={input} rows={4}
            ...
            border border-gray-300' placeholder='Describe what you wnat to see in the image...' required/>
                    ...
                    <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
                      {imageStyle.map((item)=>(
                        ...
                        className={`text-xs px-4 py-1 border rounded-full cursor-pointer
                           ${selectedStyle === item  ? 'bg-purple-50 text-green-700' : 'text-gray-700 border-gray-300'}`} 
                        key={item}>{item}</span>
                      ))}
                    </div>
  ...
  <p className='text-sm'>Make this image Public</p>
                    ...
                    <button disabled={loading} className='w-full flex justify-center items-center gap-2 bg-gradient-to-r
                     from-[#00ad25] to-[#04ff50] text-black px py-2 mt-6
      ...
      <div className='w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border
       border-gray-200 min-h-96 '>
           <div className='flex items-center gap-3'>
            ...
            <h1 className='text-xl font-semibold'>Generateed Image</h1>
           </div>
